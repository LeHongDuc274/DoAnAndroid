...
import com.example.myapplication.ui.screen.admin.ProductFormFragment.Companion.EDIT_MODE
import com.example.myapplication.ui.screen.admin.ProductFormFragment.Companion.NEW_MODE
...
class BottomSheetProductsFragment(private val activity: Context) : BottomSheetDialogFragment() {
    ...
    private fun initLitener() {
        ...
        binding.tvAddToCart.setOnClickListener {
            if(count <=0){
                activity.showToast("Lỗi. Số lượng sản phẩm bằng 0")
...
//                        }
    ...
    private fun submitNewMode() {
        ...
        if (viewModel.order.id != -1) {
            viewModel.createOrderDetail(detail) { b, mess ->
                Toast.makeText(activity, mess, Toast.LENGTH_LONG).show()
//                    list.add(data)
